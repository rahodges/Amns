...
namespace Amns.GreyFox.Web.UI.WebControls
{
	...
	public class DateEditor : System.Web.UI.Control, IPostBackDataHandler
	{
...
//		private short tabIndex;
		...
		protected void RegisterValidatorScript() 
		{
			...
			if (!Page.ClientScript.IsClientScriptBlockRegistered(ScriptKey)) 
			{
				...
				Page.ClientScript.RegisterClientScriptBlock(this.GetType(), ScriptKey, includeScript);   
			}
		}                  
		...
		protected override void Render(HtmlTextWriter output)
		{
			...
			foreach(string element in Format.Split(';'))
			{
				...
				{
					case "DAY":
						WriteSelector(output, "_day", DateStyle, "isValid('" + UniqueID + "_year', '" + UniqueID + "_month', '" + UniqueID + "_day')", 1, 31, 2, Date.Day, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "MONTH":
						WriteSelector(output, "_month", DateStyle, "isValid('" + UniqueID + "_year', '" + UniqueID + "_month', '" + UniqueID + "_day')", 1, 12, 2, Date.Month, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "YEAR":
						WriteSelector(output, "_year", DateStyle, "isValid('" + UniqueID + "_year', '" + UniqueID + "_month', '" + UniqueID + "_day')", Date.Year<_minYear && _autoAdjust ? Date.Year : _minYear, Date.Year>_maxYear && _autoAdjust ? Date.Year : _maxYear, 4, Date.Year, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "HOUR":
						WriteSelector(output, "_hour", TimeStyle, null, 0, 23, 2, Date.Hour, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "MINUTE":
						WriteSelector(output, "_minute", TimeStyle, null, 0, 59, 2, Date.Minute, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "SECOND":
						WriteSelector(output, "_second", TimeStyle, null, 0, 59, 2, Date.Second, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					case "MILLISECOND":
						WriteSelector(output, "_millisecond", TimeStyle, null, 0, 999, 3, Date.Millisecond, tabIndex);
						if(_tabBase != 0) tabIndex++;
						break;
					default:
		...
		private void WriteSelector(HtmlTextWriter output, string suffix, System.Web.UI.WebControls.Style style, string onchange, int min, int max, int padding, int selectedValue, short tabIndex)
		{
			...
			if(!this.Enabled) output.WriteAttribute("disabled", "disabled");
...
//				output.AddAttribute("tabindex", tabIndex.ToString());
		...
		public virtual bool LoadPostData(string postDataKey, NameValueCollection postCollection) 
		{
			...
			try
			{
				// The following code is bug fixed to replace illegal entries with "1" instead of the current date.
				Date = new DateTime(year == 0 ? DateTime.Now.Year : year, month == 0 ? DateTime.Now.Month : month, day == 0 ? 1 : day, hour, minute, second, millisecond);
			}
			catch
			{
