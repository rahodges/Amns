...
namespace Amns.GreyFox.Security.Web.UI.WebControls
{
	/// <summary>
	/// <summary>
	/// A delete dialog for GreyFoxRole.
	/// </summary>
	/// </summary>
	[ToolboxData("<GreyFoxRole:GreyFoxRoleDeleteDialog runat=server></{0}:GreyFoxRoleDeleteDialog>")]
	public class GreyFoxRoleDeleteDialog : TableWindow
	{

		private int greyFoxRoleID = -1;
		private GreyFoxRole greyFoxRole;
		...
		{
			GreyFoxRole greyFoxRole = new GreyFoxRole(greyFoxRoleID);
			greyFoxRole.Delete();
		...
		protected override void OnPreRender(EventArgs e)
		{
			if(greyFoxRoleID != 0)
			{
				greyFoxRole = new GreyFoxRole(greyFoxRoleID);
				text = "Delete - " + greyFoxRole.ToString();
			}
			else
			{
				text = "Delete ";
		...
		protected override void RenderContent(HtmlTextWriter output)
		{
			if(greyFoxRoleID == 0)
			{
				...
				output.Write("The GreyFoxRole selected does not exist.");
			...
			output.Write(HtmlTextWriter.TagRightChar);
			output.Write(greyFoxRole.ToString());
			output.WriteEndTag("td");
			output.WriteEndTag("tr");
