...
namespace Amns.GreyFox.Yari
{
	...
	/// </summary>
	/// </summary>
	public class YariMediaRecordCache : IEnumerable
	{
		...
		public YariMediaRecord this[int index]
		{
			get
			{
				lock(this)
				{
					...
					return yariMediaRecordArray[i];
				}
			}
		}

		public void CheckedAdd(YariMediaRecord yariMediaRecord, DateTime expiration)
		{
			lock(this)
			{
		...
		public void Add(YariMediaRecord yariMediaRecord, TimeSpan slidingExpiration)
		{
			lock(this)
			{
				count++;
		...
		private void quickSort(int left, int right)
		{
			...
			i = left;
			j = right;
			x = yariMediaRecordArray[(left + right) / 2].ID;
			while(i <= j)
			{
				while(yariMediaRecordArray[i].ID < x & i < right)
					i++;
				while(x < yariMediaRecordArray[j].ID & j > left)
					j--;
				if(i <= j)
				{
					ya = yariMediaRecordArray[i];
					yb = timeStamps[i];
					yc = absoluteExpirations[i];
					yd = slidingExpirations[i];
					yariMediaRecordArray[i] = yariMediaRecordArray[j];
					timeStamps[i] = timeStamps[j];
					absoluteExpirations[i] = absoluteExpirations[j];
					slidingExpirations[i] = slidingExpirations[j];
					yariMediaRecordArray[j] = ya;
					timeStamps[j] = yb;
		...
		private int binarySearch(int id)
		{
			...
			while (left <= right)
			{
				...
				if(id > yariMediaRecordArray[middle].ID)
					left = middle + 1;
				else if(id < yariMediaRecordArray[middle].ID)
					right = middle - 1;
				else
					return middle;
			}
