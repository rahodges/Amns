...
namespace Amns.GreyFox.Content
{
	...
	/// </summary>
	[ExposedManager("DbContentHit", "", true, 1, 1, 6234)]
	public class DbContentHitManager : IGreyFoxManager
	{
		...
		public static void CreateTableReferences()
		{
			...
			query = new StringBuilder("ALTER TABLE kitCms_Hits ADD ");
			query.Append(" CONSTRAINT kitCms_Hits_User_FK FOREIGN KEY (UserID) REFERENCES sysGlobal_Users(GreyFoxUserID),");
			query.Append(" CONSTRAINT kitCms_Hits_RequestContent_FK FOREIGN KEY (RequestContentID) REFERENCES kitCms_Clips(DbContentClipID);");
			database = DatabaseFactory.CreateDatabase();
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);
		...
		public void CreateTable()
		{
			...
			query.Append(" (DbContentHitID COUNTER(1,1) CONSTRAINT DbContentHitID PRIMARY KEY, " +
				"UserID LONG," +
				"UserAgent TEXT(255)," +
				"UserHostAddress TEXT(15)," +
				"UserHostName TEXT(255)," +
