...
namespace Amns.GreyFox.Content
{
	...
	/// </summary>
	public class DbContentClipManager
	{
		...
		internal static int _insert(DbContentClip dbContentClip)
		{
			...
			if(dbContentClip.references != null)
			{
				dbCommand = database.GetSqlStringCommand("INSERT INTO kitCms_ClipsChildren_References " +
					"(DbContentClipID, DbContentClipID)" + 
					" VALUES (@DbContentClipID, @DbContentClipID);");
		...
		internal static int _update(DbContentClip dbContentClip)
		{
			...
			if(dbContentClip.references != null)
			{
				...
				database.AddInParameter(dbCommand, "@DbContentClipID", DbType.Int32, dbContentClip.iD);
				database.AddInParameter(dbCommand, "@DbContentClipChildID", DbType.Int32);
				foreach(DbContentClip dbContentClipChild in dbContentClip.references)
				{
					dbCommand.Parameters["@DbContentClipChildID"].Value = dbContentClipChild.ID;
					database.ExecuteNonQuery(dbCommand);
				}
			}
		...
		public void CreateTableReferences()
		{
			...
			database = DatabaseFactory.CreateDatabase();
			query.Append("ALTER TABLE kitCms_Clips ADD ");
			query.Append(" CONSTRAINT FK_kitCms_Clips_Status FOREIGN KEY (StatusID) REFERENCES kitCms_Statuses (DbContentStatusID),");
			query.Append(" CONSTRAINT FK_kitCms_Clips_ParentCatalog FOREIGN KEY (ParentCatalogID) REFERENCES kitCms_Catalogs (DbContentCatalogID),");
			query.Append(" CONSTRAINT FK_kitCms_Clips_Rating FOREIGN KEY (RatingID) REFERENCES kitCms_Ratings (DbContentRatingID),");
			query.Append(" CONSTRAINT FK_kitCms_Clips_WorkingDraft FOREIGN KEY (WorkingDraftID) REFERENCES kitCms_Clips (DbContentClipID);");
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);

			query.Length = 0;
			query.Append("ALTER TABLE kitCms_ClipsChildren_References ADD");
			query.Append(" CONSTRAINT FK_kitCms_Clips_kitCms_ClipsChildren_References FOREIGN KEY (DbContentClipID) REFERENCES kitCms_Clips (DbContentClipID) ON DELETE CASCADE, ");
			query.Append(" CONSTRAINT FK_kitCms_ClipsChildren_References_kitCms_Clips FOREIGN KEY (DbContentClipID) REFERENCES kitCms_Clips (DbContentClipID) ON DELETE CASCADE;");
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);

			query.Length = 0;
			query.Append("ALTER TABLE kitCms_ClipsChildren_Authors ADD");
			query.Append(" CONSTRAINT FK_kitCms_Clips_kitCms_ClipsChildren_Authors FOREIGN KEY (DbContentClipID) REFERENCES kitCms_Clips (DbContentClipID) ON DELETE CASCADE, ");
			query.Append(" CONSTRAINT FK_kitCms_ClipsChildren_Authors_sysGlobal_Users FOREIGN KEY (GreyFoxUserID) REFERENCES sysGlobal_Users (GreyFoxUserID) ON DELETE CASCADE;");
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);

			query.Length = 0;
			query.Append("ALTER TABLE kitCms_ClipsChildren_Editors ADD");
			query.Append(" CONSTRAINT FK_kitCms_Clips_kitCms_ClipsChildren_Editors FOREIGN KEY (DbContentClipID) REFERENCES kitCms_Clips (DbContentClipID) ON DELETE CASCADE, ");
			query.Append(" CONSTRAINT FK_kitCms_ClipsChildren_Editors_sysGlobal_Users FOREIGN KEY (GreyFoxUserID) REFERENCES sysGlobal_Users (GreyFoxUserID) ON DELETE CASCADE;");
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);
		}
		...
		public void CreateTable()
		{
			...
			if (database.ConnectionStringWithoutCredentials.StartsWith("provider=microsoft.jet.oledb.4.0"))
			{
				...
				query.Append(" (DbContentClipID COUNTER(1,1) CONSTRAINT PK_kitCms_Clips PRIMARY KEY, " +
					"CreateDate DATETIME," +
					"ModifyDate DATETIME," +
					"Title TEXT(255)," +
					"Description MEMO," +
			...
			else
			{
				// Microsoft SQL Server
				query = new StringBuilder("CREATE TABLE kitCms_Clips ");
				query.Append(" (DbContentClipID INT IDENTITY(1,1) CONSTRAINT PK_kitCms_Clips PRIMARY KEY, " +
					"CreateDate DATETIME," +
					"ModifyDate DATETIME," +
					"Title NVARCHAR(255)," +
					"Description NTEXT," +
			...
			{
				query.Append("CREATE TABLE kitCms_ClipsChildren_References ");
				query.Append("(DbContentClipID INT, DbContentClipChildID INT);");
				dbCommand = database.GetSqlStringCommand(query.ToString());
				database.ExecuteNonQuery(dbCommand);
			...
			{
				query.Append("CREATE TABLE kitCms_ClipsChildren_Authors ");
				query.Append("(DbContentClipID INT, GreyFoxUserID INT);");
				dbCommand = database.GetSqlStringCommand(query.ToString());
				database.ExecuteNonQuery(dbCommand);
		...
		private static void cacheStore(DbContentClip dbContentClip)
		{
			...
			cache.Add("kitCms_Clips_" + dbContentClip.iD.ToString(), dbContentClip);
		}
		...
		private static DbContentClip cacheFind(int id)
		{
			object cachedObject;
			CacheManager cache = CacheFactory.GetCacheManager();
			cachedObject = cache.GetData("kitCms_Clips_" + id.ToString());
			if(cachedObject == null)
				return null;
		...
		{
			CacheManager cache = CacheFactory.GetCacheManager();
			cache.Remove("kitCms_Clips_" + id.ToString());
		}
        ...
        public DbContentClipCollection EncodeClips(string clips)
        {
            DbContentClipCollection encodedClips = new DbContentClipCollection();

            clips = clips.Trim();
            if (clips.Trim() == string.Empty)
                return encodedClips;
           
            string[] clipTitles = clips.Split(new string[] { "\r\n" }, 
                StringSplitOptions.RemoveEmptyEntries);

            if (clipTitles.Length < 20)
                return fastEncodeClips(clipTitles);

            DbContentClipCollection allClips = 
                GetCollection(string.Empty, string.Empty, null);

            for (int x = 0; x < allClips.Count; x++)
                for (int y = 0; y <= clipTitles.GetUpperBound(0); y++)
                    if (allClips[x].Title == clipTitles[y])
                        encodedClips.Add(allClips[x]);

            return encodedClips;
        }

        private DbContentClipCollection fastEncodeClips(string[] clipTitles)
        {
            StringBuilder query = new StringBuilder();

            for (int x = 0; x <= clipTitles.GetUpperBound(0); x++)
            {
                query.Append("DbContentClip.Title='");
                query.Append(clipTitles[x].Trim().Replace("'", "''"));
                query.Append("'");
                if (x != clipTitles.GetUpperBound(0))
                    query.Append(" OR ");
            }

            if (query.Length == 0)
            {
                throw (new Exception("No clips to encode."));
            }
		...
		public static void PeerMove(DbContentClip clip, int distance, bool enforceMenuPartition, bool menuPartitionMigration)
		{
			if(distance == 0)
				return;
			if(clip == null)
				throw(new Exception("Cannot move clip; clip is null."));

			int clipIndex = -1;
			int swapIndex = -1;
			int step;
			DbContentClipCollection peerClips;
			...
			{
				peerClips = clip.ParentCatalog.GetClips();

				// Sort clips by menu enabled to identify menu partition then by menu order
				peerClips.Sort(ContentCompareKey.MenuEnabled, ContentCompareKey.MenuOrder);
			}
			else
			{
				if(enforceMenuPartition)
					if(clip.MenuEnabled)
						peerClips = clip.ParentCatalog.GetMenuEnabledClips();
					else
						peerClips = clip.ParentCatalog.GetMenuDisabledClips();
				else
					peerClips = clip.ParentCatalog.GetClips();
			
				// Sort clips by menu order
				peerClips.Sort(ContentCompareKey.MenuOrder);
			}			

			// Find Clip Index
			for(int i = 0; i < peerClips.Count; i++)
				if(clip.ID == peerClips[i].ID)
				{
					clipIndex = i;
					break;
				}
			...
			if(clipIndex == -1)
				throw(new Exception("Cannot move clip; missing from parent catalog."));
			...
			{
				swapIndex = clipIndex + step;

				// Stop moving at boundaries of collection
				if(swapIndex < 0 | swapIndex == peerClips.Count)
					break;

				// If

				SwapMenu(peerClips[clipIndex], peerClips[swapIndex], menuPartitionMigration);
				clipIndex += step;
			}
		...
		/// <param name="a"></param>
		/// <param name="b"></param>
		public static void SwapMenu(DbContentClip a, DbContentClip b, bool menuPartitionMigration)
		{
