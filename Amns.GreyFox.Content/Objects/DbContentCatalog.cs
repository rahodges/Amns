...
namespace Amns.GreyFox.Content
{
	...
	public class DbContentCatalog : ICloneable, IComparable, IContentObject
	{
		...
		public DbContentClip NewClip()
		{
			DbContentClip c = new DbContentClip();
			
			c.ParentCatalog = this;

			c.CommentsEnabled = commentsEnabled;
			c.NotifyOnComments = false;

			c.Keywords = defaultKeywords;
			c.Status = null;													// Draft

			c.PublishDate = DateTime.Now.Add(defaultTimeToPublish);				// Deadline To Publish
			c.ExpirationDate = c.PublishDate.Add(defaultTimeToExpire);			// Expires On
			c.ArchiveDate = c.PublishDate.Add(defaultTimeToArchive);			// Archives On
			
			return c;
		}

		public DbContentClipCollection GetClips()
		{
			DbContentClipManager m = new DbContentClipManager();
			return m.GetCollection("ParentCatalogID=" + this.ID, "", null);
		}

		public DbContentClipCollection GetMenuEnabledClips()
		{
			DbContentClipManager m = new DbContentClipManager();
			return m.GetCollection("MenuEnabled=true AND ParentCatalogID=" + this.ID, string.Empty, null);
		}

		public DbContentClipCollection GetMenuDisabledClips()
		{
			DbContentClipManager m = new DbContentClipManager();
			return m.GetCollection("MenuEnabled=false AND ParentCatalogID=" + this.ID, string.Empty, null);
		}

		public DbContentCatalogCollection GetCatalogs()
		{
			DbContentCatalogManager m = new DbContentCatalogManager();
			return m.GetCollection("ParentCatalogID=" + this.ID, "", null);
		}
		
		public DbContentCatalogCollection GetMenuEnabledCatalogs()
		{
			DbContentCatalogManager m = new DbContentCatalogManager();
			return m.GetCollection("MenuEnabled=true AND ParentCatalogID=" + this.ID, string.Empty, null);
		}

		public DbContentCatalogCollection GetMenuDisabledCatalogs()
		{
			DbContentCatalogManager m = new DbContentCatalogManager();
			return m.GetCollection("MenuEnabled=false AND ParentCatalogID=" + this.ID, string.Empty, null);
		}
		...
		public GreyFoxUserCollection Authors { get { return null; } }
		public DateTime ExpirationDate { get { return DateTime.MaxValue; } }
		public DateTime PublishDate { get { return DateTime.MaxValue; } }
