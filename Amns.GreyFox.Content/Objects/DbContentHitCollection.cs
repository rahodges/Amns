...
namespace Amns.GreyFox.Content
{
	...
	public class DbContentHitCollection : IList, ICloneable 
	{
		...
		public void Sort(params HitCompareKey[] keys)
		{
			if(count > 0)
				Array.Sort(DbContentHitArray, 0, count, new HitComparer(keys));
		}

		public int CountHits(DateTime startDate, DateTime endDate, bool uniqueOnly)
		{
//			// PRESORT CODE, MAY BE FASTER
//
//			int index = 0;
//			int hitCount = 0;
//
//            Sort(HitCompareKey.RequestDate);
//
//			// Seek To Start Of Hits
//			while(index < count & DbContentHitArray[index].RequestDate < startDate) index++;
//		
//			// Count Hits
//			while(index < count & DbContentHitArray[index].RequestDate <= endDate)
//			{
//				hitCount++;
//				index++;
//			}
...
//			return hitCount;
			...
			// UNSORTED COUNT, MAY BE SLOWER

			int hitCount = 0;

			foreach(DbContentHit hit in DbContentHitArray)
			{
				// Skip non-unique hits if uniqueOnly is on
				if(uniqueOnly & !hit.IsUnique) continue;

				if(hit.RequestDate >= startDate && hit.RequestDate <= endDate)
				{
					hitCount++;
				}
			}
