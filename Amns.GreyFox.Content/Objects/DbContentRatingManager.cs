...
namespace Amns.GreyFox.Content
{
	...
	/// </summary>
	public class DbContentRatingManager
	{
		...
		public void CreateTableReferences()
		{
			...
			database = DatabaseFactory.CreateDatabase();
			query.Append("ALTER TABLE kitCms_Ratings ADD ");
			query.Append(" CONSTRAINT FK_kitCms_Ratings_RequiredRole FOREIGN KEY (RequiredRoleID) REFERENCES sysGlobal_Roles (GreyFoxRoleID);");
			dbCommand = database.GetSqlStringCommand(query.ToString());
			database.ExecuteNonQuery(dbCommand);
		}
		...
		public void CreateTable()
		{
			...
			if (database.ConnectionStringWithoutCredentials.StartsWith("provider=microsoft.jet.oledb.4.0"))
			{
				...
				query.Append(" (DbContentRatingID COUNTER(1,1) CONSTRAINT PK_kitCms_Ratings PRIMARY KEY, " +
					"CreateDate DATETIME," +
					"ModifyDate DATETIME," +
					"Name TEXT(25)," +
			...
			else
			{
				// Microsoft SQL Server
				query = new StringBuilder("CREATE TABLE kitCms_Ratings ");
				query.Append(" (DbContentRatingID INT IDENTITY(1,1) CONSTRAINT PK_kitCms_Ratings PRIMARY KEY, " +
					"CreateDate DATETIME," +
					"ModifyDate DATETIME," +
					"Name NVARCHAR(25)," +
		...
		private static DbContentRating cacheFind(int id)
		{
			object cachedObject;
			CacheManager cache = CacheFactory.GetCacheManager();
			cachedObject = cache.GetData("kitCms_Ratings_" + id.ToString());
			if(cachedObject == null)
				return null;
		...
		{
			CacheManager cache = CacheFactory.GetCacheManager();
			cache.Remove("kitCms_Ratings_" + id.ToString());
		}
